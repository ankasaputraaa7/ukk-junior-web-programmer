...
if (isset($_POST['reset_password'])) {
    ...
    if ($password_baru !== $konfirmasi) {
        ...
        // Cek keberadaan email di tabel users database
        $cek_email = mysqli_query($koneksi, "SELECT * FROM users WHERE email = '$email'");
        
        if (mysqli_num_rows($cek_email) > 0) {
            // Update password pengguna di tabel users
            $query_update = "UPDATE users SET password = '$password_baru' WHERE email = '$email'";
            if (mysqli_query($koneksi, $query_update)) {
                $message = "Kata sandi di database berhasil diperbarui! Silakan login.";
                $status_type = "success";
            } else {
                $message = "Gagal memperbarui database: " . mysqli_error($koneksi);
                $status_type = "danger";
            }
        } else {
            $message = "Email tidak terdaftar di sistem database!";
            $status_type = "danger";
        }
    }
}
...
<head>
    ...
    <style>
        ...
        .reset-card { background: #ffffff; border: 1px solid #e2e8f0; border-radius: 16px; box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.05); width: 100%; max-width: 400px; padding: 32px; }
        ...
        .btn-reset { background: linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%); color: white; font-weight: 600; border: none; border-radius: 10px; padding: 12px; width: 100%; font-size: 14px; }
    </style>
</head>
<body>
...
<div class="reset-card">
    ...
    <form action="lupa_sandi.php" method="POST">
        <div class="mb-3">
            ...
            <input type="email" name="email" class="form-control" placeholder="anka@gmail.com" required autofocus>
        </div>
        ...
        <button type="submit" name="reset_password" class="btn btn-reset mb-3">Update Kata Sandi Database</button>
